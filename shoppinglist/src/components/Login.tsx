...
function Login({loginSuccess}: LoginProps) {
  ...
  const handleGoogleLoginSuccess = async (credentialResponse : CredentialResponse) => {
    ...
    if(credentialResponse.credential) {
      setLoading(true);
      try {
        // Google ID 토큰을 백엔드로 보내서 JWT로 받아오는 과정
  ...
  const handleKeyPress = (event: KeyboardEvent) => {
    if(event.key === 'Enter' && !loading) { // 로딩 중일때는 엔터키로 로그인 못하게.
  ...
  <>
    <Stack spacing={2} alignItems="center" mt={10}>
      ...
      <Box sx={{width: '300px', display:'flex', justifyContent: 'center', opacity: loading ? 0.5 : 1}}>
        {/**Loading 중일 때는 Google 버튼을 숨기거나 비활성화. 근데 우리는 위에서 봤들이 투명도 조절*/}
        {!loading && (
          <GoogleLogin
