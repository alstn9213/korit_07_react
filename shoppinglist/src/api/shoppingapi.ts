...
const getRequestConfig = () : AxiosRequestConfig => {
  ...
  if(token) {
    return {
      headers: {Authorization: token}, // `Bearer <token>` 형식으로 저장된 토큰사용
    };
  }
  return {};
}
...
// @param id Token - Google 로그인 성공시 받은 ID 토큰
// @returns 백엔드에서 발급한 JWT(Authorization headers 값이다.)

export const authenticateWithGoogleToken = async (idToken : string) : Promise<string> => {
  console.log('Google ID 토큰을 백엔드로 전송합니다.', idToken.substring(0, 30) + '...'); // 토큰 전체 노출을 막기위해
  try {
    // 백엔드에 구현한 Google 토큰 검증 엔드포인트를 호출할것이다.
